      ...
      <main className="client-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="client-input-group">
            <label className="client-label flex items-center gap-2"><User size={14}/> {isRTL ? 'الاسم' : 'Name'}</label>
            <div className="client-input opacity-80">{profileData.name}</div>
          </div>
          <div className="client-input-group">
            <label className="client-label flex items-center gap-2"><Mail size={14}/> {isRTL ? 'الإيميل' : 'Email'}</label>
            <div className="client-input opacity-80">{profileData.email}</div>
          </div>
          <div className="client-input-group">
            <label className="client-label flex items-center gap-2"><Phone size={14}/> {isRTL ? 'الهاتف' : 'Phone'}</label>
            <div className="client-input opacity-80">{profileData.phone}</div>
          </div>
          <div className="client-input-group">
            <label className="client-label flex items-center gap-2"><Banknote size={14}/> {isRTL ? 'الدخل' : 'Income'}</label>
