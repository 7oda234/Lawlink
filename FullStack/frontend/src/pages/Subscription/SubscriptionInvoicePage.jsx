...
import { jsPDF } from 'jspdf'; // 👈 التعديل هنا (ضفنا الأقواس)
...
const SubscriptionInvoicePage = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // التعديل التاني هنا تحسباً لأي مشكلة في أبعاد الصورة
      const pdf = new jsPDF({
      ...
      pdf.save(`${invoice?.invoice_number || `invoice-${invoiceId}`}.pdf`);
  ...
  return (
    <div className={`max-w-2xl mx-auto py-16 px-4 text-center ${mode === 'dark' ? 'text-white' : 'text-slate-900'}`} dir="rtl">
      ...
      <div 
        ref={invoiceRef} 
        className={`p-8 rounded-2xl text-right mb-8 border ${mode === 'dark' ? 'bg-slate-900 border-gray-800' : 'bg-white border-gray-200'}`}
      >
        <div className="flex items-center justify-between mb-6 border-b pb-4 border-gray-500/20">
          ...
          {invoice && (
            <button 
              ...
              className="text-sm flex items-center gap-2 bg-gray-100 dark:bg-slate-800 px-4 py-2 rounded-lg hover:bg-yellow-500 hover:text-black transition-all font-bold"
            >
              <Download size={16} />
              تحميل PDF
            </button>
        ...
        {invoice ? (
          <div className="space-y-4">
            ...
            <div className="flex justify-between">
              <span className="text-gray-500">طبيعة الدفع:</span>
              <span className="font-bold text-yellow-500">
                {invoice.status === 'Partial' ? 'دفع قسط' : 'دفع كامل'}
              </span>
            </div>
          </div>
        ) : (
